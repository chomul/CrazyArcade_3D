...
## ⚠️ 유니티 빌드 — 무명 네임스페이스 이름 충돌은 커밋 후에야 드러난다

UBT 유니티 빌드는 여러 .cpp 를 `Module.*.cpp` 한 번역 단위로 합친다 → 파일마다 만든
**무명 네임스페이스가 병합**되어, 다른 .cpp 와 같은 이름의 static/헬퍼 함수가 있으면 C2084 로 터진다.

함정은 **adaptive unity**: `git status` 기준 "수정 중인 파일"은 유니티에서 빠져 단독 컴파일된다.
즉 작업 중에는 빌드가 통과하고, **커밋해서 워킹 트리가 깨끗해진 순간** 유니티로 합류하며 실패한다
