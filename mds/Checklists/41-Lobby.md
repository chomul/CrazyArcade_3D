...
- [x] **패키징 실전 검증 (2026-08-16, 사용자 에셋 제작 후)** — 서버·클라 재패키징 후 실제 접속:
      · 서버: `로비 페이즈 시작 — 방장의 시작 요청까지 대기` · `방장 배정 — <이름> (첫 입장)` ·
        `로비 페이즈 중 입장 — 폰 스폰 보류 (대기 1명)` (스폰 게이트 연장이 실제로 걸렸다)
      · 클라: `로비 위젯 표시 — WBP_Lobby_C` · **미바인딩 위젯 경고 0건**
        (= 바인딩 5개 전부 연결. `BindWidgetOptional` 이라 경고 부재가 유일한 증거)
      · 양쪽 `LogCA3D` Error 0

## 남은 검증 (미실행 — 체크 금지)

- [ ] 클라 **2대**: 둘째에게 준비 버튼 · 전원 준비 전 시작 버튼 비활성 → 준비 → 활성 →
      시작 → 캐릭터 선택 → 매치 (`mds/packaged-test.md`)
- [ ] 로비 중 마우스 커서로 버튼이 실제로 눌리는가 (사람 손이 필요 — 헤드리스로는 불가)
