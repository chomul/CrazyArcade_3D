...
## 빌드 (필수 게이트)
- [x] `CrazyArcade3DEditor` 빌드 통과 (2026-07-30 · 유니티 병합 강제 `-ForceUnity -DisableAdaptiveUnity`)
...
## 보류 — 규칙 확정 필요
- [ ] **중도 이탈(`Logout`)**: "이탈 = 그 자리에서 탈락(순위 부여)" vs "참가 인원·AliveCount 에서 제외(순위 없음)".
      `CA3DGameMode::PostLogin` 아래 TODO 주석으로 보존. 확정 전에 구현하면 승패 판정이 조용히 어긋난다
