...
- [x] 블록 파괴 + 렌더 갱신 + 그 위 캐릭터 낙하 (설계서 5장 7번) — 파괴·그리드 반영은 자동 해시 로그 10회(솔리드 756→740칸)로 확인, 렌더 갱신·낙하는 사용자 PIE 확인 (2026-07-30)
- [x] 물줄기 피격 → Trap → 익사 (5장 10번) — 로그: `갇힘 — 4.0초 후 익사` → `사망 — 원인 EDeathCause::Water` (두 플레이어 모두, 2026-07-30)
- [ ] 위 항목의 점프 판정 세부: **제자리 점프는 피격, 다른 발판 점프는 회피** — 미검증 (발밑 셀 정의가 잠정이라 확정 시 재확인)
...
- [x] 폭탄 10개 연쇄 `stat unit` 스파이크 없음 (5장 8번) — 사용자 PIE 확인 (2026-07-30 · 이상 보고 없음). 프레임 수치는 로그 미기록 — 성능 회귀가 의심되면 재측정할 것
