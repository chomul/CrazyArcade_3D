...
- [x] **🐛 우승 매치가 "무승부"로 표시되던 버그 수정 (2026-08-06 사용자 발견)** —
      ...
      수정(사용자 방향 확정 — "결과가 오기 전엔 그리지 말 것"): **판정을 GameState 로 옮겼다.**
      `ACA3DGameState::MatchWinner`(복제, nullptr=무승부)를 GameMode 가 `bMatchEnded` 와 동시에
      쓴다 — **같은 액터의 프로퍼티는 한 번들로 원자 복제**되므로 클라는 종료를 아는 프레임에
      승패도 반드시 함께 안다. 기다림도, 자가 수정도, 타임아웃도 필요 없다.
      (행 채움은 매 틱 갱신 유지 — 랭크가 늦으면 "-등"이 다음 틱에 채워질 뿐 승패 문구는 불변.
      "전원 랭크 도착까지 대기" 방식을 쓰지 않은 이유: 랭크가 영영 없는 관전자(후속 Task)가
      생기는 순간 결과 화면이 영영 안 뜬다.)
      회귀: `CA3DPlayerState` 테스트에 MatchWinner 어서션 3개 추가 (우승 2케이스 + 무승부).
      ⚠️ 2026-08-04 의 "무승부 규약 첫 발동" 기록은 이 버그였을 가능성이 높다 —
      진짜 무승부(전원 동시 사망)는 아직 실전에서 관측되지 않았다
