...
- [x] **🐛 우승 매치가 "무승부"로 표시되던 버그 수정 (2026-08-06 사용자 발견)** —
      ...
      **실전 확인 (2026-08-06 `-game` 4인)**: 서버 `우승자 확정` ↔ 위젯 `우승자 있음` 일치.
      ⚠️ 2026-08-04 의 "무승부 규약 첫 발동" 기록은 이 버그였을 가능성이 높다 —
      진짜 무승부(전원 동시 사망)는 아직 실전에서 관측되지 않았다
- [x] **결과 화면 첫 줄에 내 순위 (2026-08-06 사용자 요청)** — 목록만 있으면 자기 이름을
      찾아야 등수를 알 수 있어 한눈에 안 들어온다는 지적. 첫 줄은 `우승!` / `내 순위  N등` /
      `무승부`, 그 아래 전체 순위이고 본인 행에는 `▶` 표식이 붙는다.
      · 본인 판별은 **이름이 아니라 PlayerState 포인터** — 봇 기본 이름이 겹치면 엉뚱한 행에
        표식이 붙는다
      · 무승부가 등수보다 우선 — 전원 공동 2등이라 `공동 2등` 만 보이면 이긴 줄 안다
      · 관전자·PlayerState 미도착이면 헤드라인만 `매치 종료` 로 떨어지고 목록은 정상
