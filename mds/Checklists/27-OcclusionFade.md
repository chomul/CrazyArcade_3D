...
- [x] **`OcclusionFadeAmount` = 1.0 확정** (2026-08-06 사용자 체감 — "훨씬 깔끔").
      0.8 이면 디더가 픽셀 20%를 남기고, 그 잔존 픽셀이 구멍 안에 흩뿌려져 **노이즈로 보인다.**
      게다가 마스크가 캐릭터를 따라 매 프레임 움직여서 **TSR 이 누적으로 지워 주지도 못한다**
      (정지 물체의 디더 LOD 전환은 잘 되는데 이건 안 되는 이유). 1.0 이면 중심부가 완전히 비고
      디더는 가장자리 띠에만 남아 오히려 경계를 부드럽게 한다. C++ 기본값도 1.0 으로 변경
- [ ] `OcclusionMaskScale`(1.35) · `OcclusionMaskSoftness`(0.4) 는 현행 유지 — 플레이하며 재확인
