...
- [x] **(후속) 킥 실제 차기 동작** — `ABomb` 킥 미끄러짐 + `ACA3DCharacter::ServerTryKickBomb` +
      ...
      · ⚠️ **미결**: 찬 폭탄이 낭떠러지로 떨어지는가 (지금은 공중을 가로질러 미끄러짐 — 사용자 확인 대기)
...
- [x] **(후속) 사망 후 관전** — `ACA3DPlayerController` 에 생존자 추적·순환 +
      ...
      · ⚠️ **봇 `ControlRotation` pitch 를 룰셋 각도로 덮어씀** — `AAIController` 기본 구현이
        "상대를 안 보면 pitch=0" 이라 **봇을 관전하면 카메라가 지면에 눕는다**. 캐릭터가
        `bUseControllerRotationYaw=false` 라 봇 동작에는 영향 0
