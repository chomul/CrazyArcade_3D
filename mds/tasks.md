...
- [ ] **22. `UProcMapGenerator`** — 지금은 고정 맵 하나뿐.
      **확정(2026-08-06 사용자)**: 지형 기복 = **산·협곡까지(Z 6층으로 확장)** ·
      맵 크기 = **인원별 티어**(값은 룰셋 DataAsset 관리).
      ⚠️ 크기가 가변이면 **크기도 복제해야 한다** — 클라가 로컬 인원수로 크기를 유추하면
      중간 접속에서 어긋난다 (알려진 함정 "중간 접속"과 같은 사정)
