...
**WASD = 카메라 기준** · **이동 가감속 0.05초**(공중 포함, AirControl 1.0) · **카메라 붐 컬리전 off**(가림은 3주차 디더 페이드) ·
**갇힘 중 점프 금지**(이동은 미세 이동만 허용) · **공중 폭탄 설치 = 바로 아래로 내려 찾은 셀, 피격 = 셀 단위**(잠정 — 체감 후 재확인) ·
**폭탄은 플레이어를 막는다**(설치 직후엔 Overlap → 겹친 폰이 전부 빠져나가면 Block 승격. 컬리전은 데디에서도 살려 둔다)
