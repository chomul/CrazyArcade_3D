...
버전 관리는 **이중**: Diversion(사용자가 직접 관리) + git(Claude가 각 Task 후 커밋 — 불변식 6). git push는 리모트가 생기기 전까지 없음.
...
- 셀 크기 / 이동속도 / 점프 높이 · 공중에 있을 때 "발밑 셀"의 정의
- 카메라 입력 기준 — WASD가 월드 축 기준인가 카메라 기준인가 (45도 스냅 회전과 맞물림)
