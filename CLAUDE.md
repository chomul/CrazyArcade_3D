...
확정된 값: **점프 높이 = 1칸**(2칸은 못 오름) · **WASD = 카메라 기준**(2026-07-30 확정 — `ca3d.CameraRelativeInput` 0 으로 월드축 비교 가능) ·
**이동 가감속 = 0.05초**(즉시 반응 — 룰셋의 시간 값에서 파생) · **카메라 붐 컬리전 off**(벽에서 확대되는 문제 — 가림은 3주차 디더 페이드) ·
