...
- 셀 크기 — 임시 100. 에셋·캐릭터 크기 보고 확정 예정, **유동적으로 유지** (파생 값은 셀 크기 기준 계수로)
- 이동속도 — "1초에 몇 칸" 계수로 튜닝, 수치 미확정
- 바닥 블록 파괴 허용 여부 / 스폰 무적 시간 / 아이템 스택 상한

확정된 값 (2026-07-29, 잠정 포함): **점프 높이 = 1칸**(2칸은 못 오름) · **WASD = 월드 축 기준**(잠정 — 카메라 기준 토글 유지) ·
**공중 폭탄 설치 = 자기 바로 아래로 내려 찾은 셀 기준, 피격 판정 = 폭발 범위를 셀 단위로**(잠정 — Task 16에서 구현·체감 후 재확인)
