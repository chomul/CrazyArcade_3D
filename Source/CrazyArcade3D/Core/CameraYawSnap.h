...
// 스냅 카메라 yaw 의 **인덱스 ↔ 각도 변환 단일 출처**.
//
// **한 칸 = 90도** (2026-08-09 사용자 확정 — 45도에서 변경). 90도만 쓰면 카메라 축이 항상
// 그리드 축과 나란해서, 카메라 기준 WASD 가 언제나 칸 방향으로 정확히 떨어진다.
// 45도에서는 절반의 각도에서 W 가 격자를 대각선으로 가로질러 "한 칸 앞" 이 어디인지가 흐려졌다.
//
...
{
	// 4방향 고정 — **튜닝 값이 아니라 구조 상수**다. 복제되는 인덱스가 uint8 0~3 인 것도,
	// 스텝이 90도인 것도 전부 이 값 하나에서 나온다.
	// 룰셋에 두지 않는 이유: 런타임에 바뀌면 이미 복제된 인덱스의 의미가 통째로 달라진다.
	inline constexpr int32 NumSteps = 4;
	inline constexpr float StepDeg = 360.f / static_cast<float>(NumSteps); // 90

	// 누적 스텝 → 인덱스 0~3. 컨트롤러는 스텝을 랩하지 않고 계속 더하거나 빼므로
	// (보간이 FRotator 정규화로 최단 경로를 택한다) 음수도 들어온다.
	uint8 StepsToIndex(int32 Steps);

	// 인덱스 → 각도(도). 0, 90, 180, 270.
	float IndexToYawDeg(uint8 Index);

	// 임의 각도 → 가장 가까운 인덱스. IndexToYawDeg 와 정확히 왕복한다.
	uint8 YawDegToIndex(float YawDeg);

	// 히스테리시스판 — 현재 인덱스에서 벗어나려면 경계(StepDeg/2 = 45도)를 HysteresisDeg 만큼
	// **더** 넘어야 한다. 봇처럼 목표각이 연속적으로 미세하게 흔들리는 입력에 쓴다:
