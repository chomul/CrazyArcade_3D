...
- 블록이 부서져 **노출된 아이템 1개**를 표현한다: 표시(메시·회전) + 획득 판정(스피어).
- 획득 시 효과 적용을 `StatusComponent`에 위임하고 소멸, 물줄기에 맞으면 효과 없이
  소멸(`ServerBurn`) — 두 퇴장 경로를 구분해 기록한다.
- 서브시스템 레지스트리에 자신을 등록해 봇이 셀로 찾을 수 있게 한다.
