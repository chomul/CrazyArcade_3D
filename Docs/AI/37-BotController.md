...
- **Q. BP 허용이라면 FSM vs BT+블랙보드 뭘 쓸 거고 장단점은?** → 그래도 **FSM.**
  BT의 장점은 전부 "사람 여럿·행동 많음"에서 빛나는데 이 프로젝트는 솔로·상태 5개·
  봇 1종·3주라 해당 없음. FSM 장점(디버깅·테스트·diff)은 솔로+단기에서 매일 쓰는 것.
  **FSM** — 장: 중단점 하나로 "왜 이 행동?" 즉답 / 순수 함수 조합이라 유닛 테스트 /
  if 순서 = 우선순위 명세 / 텍스트라 diff·리뷰.
  단: 상태 ~10개 넘으면 전이 스파게티 / 비프로그래머 수정 불가 / 인터럽트·병렬 수작업.
  **BT+BB** — 장: 행동 계층이 그림(협업·온보딩) / Observer abort로 인터럽트 공짜 /
  태스크 조각 재사용으로 봇 변종 양산 / Perception·EQS 통합.
  단: 에셋=바이너리(diff·머지·리뷰 불가) / 흐름이 노드에 흩어져 디버깅 난해 /
  블랙보드 키 오타가 런타임에야 발각 / 테스트 불가 + 태스크마다 보일러플레이트.
  **전환 기준**: 상태 10개↑ · 디자이너 튜닝 필요 · 봇 변종 다수 — 하나라도 오면 BT.
  그때도 판정 함수들(`ShouldPlaceBombAt` 등)은 BT 태스크가 그대로 호출 —
  이식 비용은 골격뿐, 뇌는 재사용
