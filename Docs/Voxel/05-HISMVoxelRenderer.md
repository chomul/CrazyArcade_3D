...
- 그리드의 표면 셀을 타입별 HISM 인스턴스로 **생성·제거**한다 (`IVoxelRenderer` 구현).
- 셀↔인스턴스 인덱스 양방향 맵을 유지한다 — 파괴·페이드가 올바른 인스턴스를 찾는 근거.
- 페이드 값을 인스턴스 커스텀 데이터로 전달한다(`SetCellFade`).
- **부수적이지만 치명적인 역할**: 이 HISM 인스턴스들이 지형의 유일한 물리 컬리전이다.
