...
- **Q. 그리드는 서버에만 있고, 그 값으로 서버·클라가 렌더러를 각각 돌리나?** →
  "각자 렌더러"는 맞지만 **그리드 사본도 양쪽에 각자 있다.** 전송되는 건 데이터가 아니라
  재료(시드 + 파괴 이력)이고, 클라는 같은 생성기·같은 `ApplyDestruction`으로 자기 사본을
  만들어 유지한다. 진실은 서버 사본뿐 — 판정은 전부 서버 그리드, 클라 사본은
  렌더 입력 + 위험 프리뷰(Propagate 로컬) + 설치 예측 검증 + 가림 페이드용.
  사본 일치의 전제가 불변식 4(결정론)이고 해시 30건 일치로 실측 증명됨
