...
- **Q. VoxelWorld가 가진 스태틱 메시를 렌더러가 그려주는 건가?** → 소유가 반대.
  **메시 에셋은 렌더러가 소유**(`BlockMeshes` — BP에서 지정)하고 VoxelWorld에는 메시
  참조가 한 줄도 없다(숫자 배열뿐). 렌더러는 그리드 숫자를 자기 메시로 번역해 HISM
  인스턴스를 만들고, 실제 그리기는 엔진 몫. 메시가 렌더러 쪽인 이유: 메시 선택 =
  표현의 관심사 — 그리디 메싱으로 바꾸면 "블록당 큐브" 개념 자체가 사라지는데,
  표현 결정이 전부 렌더러 안에 있어야 교체가 한 줄로 끝남
