...
- **HUD 3요소 표시**: 생존 수·경과 시간 / 내 스탯(폭탄·범위·속도·니들·킥) / 서든데스 경고.
- **결과 화면**: 순위 정렬·공동 등수·무승부·탈주 표시·내 순위 헤드라인.
- 방법: 매 틱 복제 값을 **폴링** → 스냅샷 비교로 변한 것만 → static 순수 함수로 문자열 가공
  → 위젯 갱신. 레이아웃은 WBP(BP 서브클래스)가, 내용은 C++이.
