...
- **매치 시작 준비**: 시드 결정 → VoxelWorld 초기화(크기 티어) → 지형 준비 전 입장자
  대기·해소(스폰 게이트) → 스폰 셀 순서 배정.
- **참가 관리**: 사람·봇 공용 등록(`RegisterParticipant`) · 봇 채우기 · 중도 이탈 처리.
- **승패 판정**: 사망 통지 수집(`NotifyPlayerDeath`) → 다음 틱 배칭(`ResolvePendingDeaths`)
  → 공동 등수·우승·무승부 확정 → GameState에 기록.
- **서든데스 스위치**: 시작 시각 도달 시 서브시스템 기동, 매치 종료 시 정지.
